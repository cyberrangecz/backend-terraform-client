...
from kypo.openstack_driver.transformation_configuration \
    import TransformationConfiguration
from kypo.openstack_driver.exceptions import KypoException
from kypo.openstack_driver.topology_elements \
    import MAN, Node, Link, NodeToNodeLinkPair
...
class TopologyInstance:
    ...
    def _create_extra_nodes_links(self) -> None:
        ...
        """
        self._add_link(self.man, self.man_network)
        self._add_link(self.man, self.wan)
    ...
    def _create_hosts_links(self, topology_definition_net_mappings: NetworkMappingList) -> None:
        ...
        for host in self.get_hosts():
            self._add_link(host, self.man_network)
        ...
        for network_mapping in topology_definition_net_mappings:
            ...
            self._add_link(host, host_network, ip=network_mapping.ip)
    ...
    def _create_routers_links(self, topology_definition_routers: RouterList,
        ...
        for router in topology_definition_routers:
            self._add_link(router, self.man_network)
            self._add_link(router, self.wan)
        ...
        for router_mapping in topology_definition_router_mappings:
            ...
            self._add_link(router, host_network, router_mapping.ip)
